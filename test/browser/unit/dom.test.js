...
describe('dom:', function() {
	describe('$', function() {
		...
		// problem testing exceptions in ie 8-9
		if (!browser.isIE8 && !browser.isIE9) {
		...
		it('should return null if none found', function() {
			printTestname(this);
			var found = win.AdServ.$ID('non_found');
			expect(found).to.be(null);
		});

		it('should be able to find the elem by id', function() {
			printTestname(this);
			var found = win.AdServ.$ID('group1');
		...
		it('should be able read the used css properties', function() {
			printTestname(this);
			var elem = win.AdServ.$ID('with-style');
			if (browser.isIE8) {
			...
			elem = win.AdServ.$ID('without-style');
